...
static void select_row(uint32_t row) {
    setPinOutput(row_pins[row]);
    writePinLow(row_pins[row]);
}

static void unselect_row(uint32_t row) { 
    setPinInputHigh(row_pins[row]); 
}

static void unselect_rows(void) {
    for (uint32_t x = 0; x < MATRIX_ROWS; x++) {
        setPinInputHigh(row_pins[x]);
    }
}

static void select_col(uint32_t col) {
    setPinOutput(col_pins[col]);
    writePinLow(col_pins[col]);
}

static void unselect_col(uint32_t col) {
    setPinInputHigh(col_pins[col]);
}

static void unselect_cols(void) {
    for (uint32_t x = 0; x < MATRIX_COLS/2; x++) {
        setPinInputHigh(col_pins[x*2]);
    }
}


static void read_cols_on_row(matrix_row_t current_matrix[], uint32_t current_row) {
    // Select row and wait for row selection to stabilize
    select_row(current_row);
    wait_us(MATRIX_IO_DELAY);

    // For each col...
    for (uint32_t col_index = 0; col_index < MATRIX_COLS / 2; col_index++) {
        uint32_t column_index_bitmask = ROW_SHIFTER << ((col_index * 2) + 1);
        // Check row pin state
...
static void read_rows_on_col(matrix_row_t current_matrix[], uint32_t current_col) {
    // Select col and wait for col selection to stabilize
    select_col(current_col*2);
    wait_us(MATRIX_IO_DELAY);

    uint32_t column_index_bitmask = COL_SHIFTER << (current_col * 2);
    // For each row...
    for (uint32_t row_index = 0; row_index < MATRIX_ROWS; row_index++) {
        // Check row pin state
...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    // Set row, read cols
    for (uint32_t current_row = 0; current_row < MATRIX_ROWS; current_row++) {
        read_cols_on_row(current_matrix, current_row);
    }

    // Set col, read rows
    for (uint32_t current_col = 0; current_col < MATRIX_COLS / 2; current_col++) {
