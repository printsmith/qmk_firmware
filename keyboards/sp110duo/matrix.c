...
static void select_row(uint16_t row) {
...
static void select_col(uint16_t col) {
    setPinOutput(col_pins[col]);
    writePinLow(col_pins[col]);
}

static void unselect_col(uint16_t col) {
    setPinInputHigh(col_pins[col]);
}

static void unselect_cols(void) {
    for (uint16_t x = 0; x < MATRIX_COLS/2; x++) {
        setPinInputHigh(col_pins[x*2]);
    }
}


static void read_cols_on_row(matrix_row_t current_matrix[], uint16_t current_row) {
...
static void read_rows_on_col(matrix_row_t current_matrix[], uint16_t current_col) {
    ...
    // For each row...
    for (uint16_t row_index = 0; row_index < MATRIX_ROWS; row_index++) {
        // Check row pin state
...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    // Set row, read cols
    for (uint16_t current_row = 0; current_row < MATRIX_ROWS; current_row++) {
        read_cols_on_row(current_matrix, current_row);
    }

    // Set col, read rows
    for (uint16_t current_col = 0; current_col < MATRIX_COLS / 2; current_col++) {
